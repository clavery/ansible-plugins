...
GPG_FILENAME_EXTENSIONS = ['.yaml.gpg', '.yml.asc', '.yml.gpg', '.yaml.asc']
...
def _load_vars(basepath, results, vault_password=None):
    ...
    paths_to_check = ["".join([basepath, ext])
                      for ext in GPG_FILENAME_EXTENSIONS]
...
class VarsModule(object):
    ...
    """
    Loads variables from GPG encrypted files in a secrets/ directory.

    Group vars are loaded from secrets/group_vars and host vars from secrets/host_vars
    """
    ...
    def run(self, host, vault_password=None):
        ...
        for basedir in [inventory_basedir, self.pb_basedir]:
            ...
            for group in groups:
                base_path = os.path.join(basedir, "secrets", "group_vars/%s" % group)
                results = _load_vars(
                    base_path, results, vault_password=vault_password)

            # same for hostvars in dir/host_vars/name_of_host
            base_path = os.path.join(basedir, "secrets", "host_vars/%s" % host.name)
            results = _load_vars(
                base_path, results, vault_password=vault_password)
